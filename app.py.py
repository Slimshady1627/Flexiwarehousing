...
# Helper function to grab database connections safely
def get_db_connection():
    return sqlite3.connect("warehouse_marketplace.db")

# ==========================================
# 2. APP HEADER & LAYOUT SWITCHER
# ==========================================
...
# Top Navigation Tabs simulating different user portals
...
with tab1:
    ...
    with col3:
        min_space = st.number_input("Minimum Space Needed (Sq. Ft.)", min_value=0, value=0, step=500)
        
    # Build dynamic SQL query based on filters
    conn = get_db_connection()
    query = "SELECT * FROM warehouses WHERE available_capacity >= ?"
    params = [min_space]
        ...
        for index, row in df_warehouses.iterrows():
            with st.container(border=True):
                ...
                with c3:
                    ...
                    # Expansion drawer for making a booking request
                    with st.popover("Book Space"):
                        ...
                        if st.button("Submit Booking Request", key=f"btn_{row['id']}", type="primary"):
                            if renter_name:
                                ...
                                conn.commit()
                                conn.close()
                                st.success("🎉 Request submitted successfully! Our backend team will contact you shortly.")
                            else:
...
with tab2:
    ...
    st.write("Monetize your unutilized space by putting it on our network.")
    
    with st.form("host_onboarding_form", clear_on_submit=True):
        col_a, col_b = st.columns(2)
        with col_a:
            owner = st.text_input("Owner / Authorized Signatory Name")
            loc = st.text_input("Exact Facility Address / Logistics Hub Hub Location")
        ...
        if submit_listing:
            if owner and loc:
                ...
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO warehouses (owner_name, location, total_capacity, available_capacity, storage_type, price_per_sqft)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (owner, loc, tot_cap, tot_cap, st_type, price))
                conn.commit()
                conn.close()
                st.success("✨ Your warehouse layout configuration has been recorded. It will show live on the marketplace as soon as the Admin team reviews and verifies the facility safety documents.")
            else:
                st.error("Please fill out all identity and address fields before submitting.")

# ==========================================
# TAB 3: ADMIN CONTROLS (YOUR BACKEND MANAGEMENT)
# ==========================================
with tab3:
    st.header("Internal Operator Management Dashboard")
    st.write("Use this screen to manage physical inspections, approve logistics listings, and update manual escrows.")
        ...
        for idx, row in df_unverified.iterrows():
            with st.container(border=True):
                ...
                with cx:
                    ...
                    st.caption(f"Specs: {row['storage_type']} — {row['total_capacity']:,} Sq. Ft. at ₹{row['price_per_sqft']}/Sq. Ft.")
                with cy:
