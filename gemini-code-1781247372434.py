...
def init_db():
    ...
    # Warehouses Table (Updated with Loading/Unloading Mechanism)
    cursor.execute("""
...
with tab1:
    ...
    # Search Filters Column Layout (Expanded to 5 columns for logistics precision)
    ...
    with col5:
        dock_type = st.selectbox("Loading Infrastructure", ["All", "Hydraulic Dock Levelers", "Manual Dock / Ramps", "Ground Level (No Dock)"])
...
with tab2:
    ...
    with st.form("host_onboarding_form", clear_on_submit=True):
        ...
        with col_b:
            ...
            # New dropdown feature for loading bay equipment
            load_mech = st.selectbox("Bay Loading/Unloading Mechanism", ["Hydraulic Dock Levelers", "Manual Dock / Ramps", "Ground Level (No Dock)"])
            ...
            if owner and loc:
                if avail_cap > tot_cap:
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (owner, loc, tot_cap, avail_cap, c_height, f_load, load_mech, st_type, price))
                    conn.commit()
                    conn.close()
                    st.success("✨ Warehouse listing including localized loading bay profiles recorded successfully.")
            else:
