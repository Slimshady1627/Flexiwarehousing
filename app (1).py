...
with tab2:
    ...
    with st.form("host_onboarding_form", clear_on_submit=True):
        ...
        with col_a:
            ...
            storage_type = st.selectbox("Storage Category Profile", ["Ambient", "Cold Storage", "Pallet Racks", "Open Yard"])
            ...
            if not selected_lat or not selected_lng:
                ...
                if avail_cap > tot_cap:
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (owner, loc, float(selected_lat), float(selected_lng), tot_cap, avail_cap, c_height, f_load, final_mechanism, storage_type, price))
                    conn.commit()
                    conn.close()
