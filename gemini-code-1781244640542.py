...
def init_db():
    ...
    # Warehouses Table (Updated with Clear Height & Floor Load)
    cursor.execute("""
...
with tab1:
    ...
    # Search Filters Column Layout (Expanded to 4 columns)
    ...
    with col4:
        min_height = st.number_input("Min Clear Height Needed (Ft.)", min_value=0, value=0, step=5)
        ...
        for index, row in df_warehouses.iterrows():
            with st.container(border=True):
                ...
                with c1:
                    ...
                    st.markdown(f"📐 **Clear Height:** {row['clear_height']} ft. | 🏗️ **Floor Load:** {row['floor_load']} Tons/Sq.M.")
                with c2:
...
with tab2:
    ...
    # Corrected form submission button implementation
        ...
        if submit_listing:
            if owner and loc:
                if avail_cap > tot_cap:
                    ...
                    cursor = conn.cursor()
                    cursor.execute("""
                        INSERT INTO warehouses (owner_name, location, total_capacity, available_capacity, clear_height, floor_load, storage_type, price_per_sqft)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """, (owner, loc, tot_cap, avail_cap, c_height, f_load, st_type, price))
                    conn.commit()
                    conn.close()
                    st.success("✨ Your warehouse configuration (including vertical and floor parameters) has been recorded successfully.")
            else:
        ...
        for idx, row in df_unverified.iterrows():
            with st.container(border=True):
                ...
                with cx:
                    ...
                    st.caption(f"Specs: {row['storage_type']} — Height: {row['clear_height']}ft | Floor Load: {row['floor_load']}T/m² | Available Space: {row['available_capacity']:,} Sq. Ft.")
                with cy:
